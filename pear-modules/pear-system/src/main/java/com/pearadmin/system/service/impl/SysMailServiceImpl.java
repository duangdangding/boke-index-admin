...
public class SysMailServiceImpl implements ISysMailService {
    ...
    public Integer save(SysMail sysMail) {
        if (sendMail(sysMail)) {
            ...
            sysMail.setCreateBy( SecurityUtil.currentUser().getUsername());
    ...
    @Override
    public IPage<SysMail> page(SysMail sysMail, PageDomain pageDomain) {
        Page<SysMail> page = new Page<>(pageDomain.getPage(), pageDomain.getLimit());
        return sysMailMapper.selectList(page,sysMail);
    }
/*
    @Override
