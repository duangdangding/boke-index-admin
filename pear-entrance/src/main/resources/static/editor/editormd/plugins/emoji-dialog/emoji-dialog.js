...
(function() {
	...
	var factory = function (exports) {
		...
		exports.fn.emojiDialog = function() {
            ...
            var emojiCategories = ["we_emoji","github-emoji","twemoji", "font-awesome"];
			...
			var drawTable = function() {
                ...
                var pagination = function(data, type,category) {
                    var rowNumber = (type === "we_emoji") ? "9" : 20;
                    ...
                    for (let i = 0; i < pageTotal; i++) {
                        ...
                        for (let x = 0; x < rowNumber; x++) {
                            let emoji = $.trim(data[(i * rowNumber) + x]);
                            ...
                            if (typeof emoji !== "undefined" && emoji !== "") {
                                ...
                                if (type === "we_emoji") {
                                    ...
                                    src     = exports.we_emoji.path + src;
                                    let w = 80;
                                    if (category && category === 'QQ') {
                                        w = 24;
                                    }   
                                    let val_img = '![]('+src+')';
                                    // img     = "<img src=\"" + src + "\" width='"+w+"' class='emoji' title=\"&#58;" + emoji + "&#58;\" alt=\"&#58;" + emoji + "&#58;\" />";
                                    img     = "<img src=\"" + src + "\" width='"+w+"' class='emoji' title=\"&#58;" + emoji + "&#58;\" />";
                                    /*let bigImg = '<span><img src="'+src+'" alt="big" /></span>';
                                    img = img + bigImg;*/
                                    row += '<a href="javascript:;" value="'+val_img+'" title="'+val_img+'" class="'+classPrefix+'emoji-btn d_imgbox">'+img+'</a>';
                                    // row += "<a href=\"javascript:;\" value=\":" + emoji + ":\" title=\":" + emoji + ":\" class=\"" + classPrefix + "emoji-btn\">" + img + "</a>";
                                } else if (type === "github-emoji") {
                                    let src = (emoji === "+1") ? "plus1" : emoji;
                                    src     = (src === "black_large_square") ? "black_square" : src;
                                    src     = (src === "moon") ? "waxing_gibbous_moon" : src;
                ...
                if (emojiTabIndex === 0) {
                    for (var i = 0, len = $data.length; i < len; i++) {
                        ...
                        $tab.append(pagination($data[i].list, cname,$data[i].category));
                    }
                } else if (emojiTabIndex === 1) {
                    for (var i = 0, len = $data.length; i < len; i++) {
                        var h4Style = (i === 0) ? " style=\"margin: 0 0 10px;\"" : " style=\"margin: 10px 0;\"";
                        $tab.append("<h4" + h4Style + ">" + $data[i].category + "</h4>");
