...
(function ($) {
	...
	function init(o){
        ...
        $(o.searchName+" .layui-input").click(function(event){
			...
			$(this).siblings("ul").find("li").click(function(){
				...
				$("#cateitemSelect").val($(this).attr("value"));
			});
		...
		$(o.searchName).keydown(function(event){
			...
			if(event.keyCode == 13){
				...
				// $(o.searchName).find("#cateitemSelect").val(select_v);
				$("#cateitemSelect").val(select_v)
			}
	...
	function selectKeyUD(event, name){
		...
		select_v = $(name+" ul").find(".on").eq(SELECT_INDEX).attr("value");
		// console.log(select_v + "~" + SELECT_VALUE)
